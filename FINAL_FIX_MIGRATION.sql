-- ============================================================================
-- FINAL COMPREHENSIVE FIX - Updates existing data then applies constraints
-- ============================================================================

-- ============================================================================
-- STEP 1: Fix existing data in orders table to match new constraints
-- ============================================================================

-- Update any non-standard guest_type values to match our constraint
UPDATE public.orders 
SET guest_type = CASE 
  WHEN guest_type ILIKE '%walk%' OR guest_type ILIKE '%walkin%' THEN 'walk-in'
  WHEN guest_type ILIKE '%hotel%' OR guest_type ILIKE '%guest%' THEN 'hotel-guest'
  ELSE 'walk-in'  -- Default fallback
END
WHERE guest_type NOT IN ('walk-in', 'hotel-guest');

-- Update any non-standard status values to match our constraint
UPDATE public.orders 
SET status = CASE 
  WHEN status ILIKE '%complete%' OR status ILIKE '%paid%' THEN 'paid'
  WHEN status ILIKE '%cancel%' THEN 'cancelled'
  WHEN status ILIKE '%pending%' OR status ILIKE '%active%' THEN 'active'
  ELSE 'active'  -- Default fallback
END
WHERE status NOT IN ('active', 'paid', 'cancelled');

-- ============================================================================
-- STEP 2: Now drop and recreate constraints on orders table
-- ============================================================================
...
-- ============================================================================
-- STEP 3: Add foreign key from order_items to orders
-- ============================================================================
...
-- ============================================================================
-- STEP 5: Create gym_members table
-- ============================================================================
...
-- ============================================================================
-- STEP 6: Create gym_equipment table
-- ============================================================================
...
-- ============================================================================
-- STEP 7: Create gym_trainers table
-- ============================================================================
...
-- ============================================================================
-- STEP 8: Create gym_trainer_bookings table
-- ============================================================================
...
-- ============================================================================
-- STEP 9: Ensure all helper functions exist
-- ============================================================================
...
SELECT '✓ Migration completed successfully! All tables created and constraints fixed.' AS result;
