-- =====================================================
-- COMPLETE DATABASE SETUP FOR OWEN'S DATABASE
-- =====================================================
...
-- Step 3: Create all core tables with proper schema
...
-- Step 7: Drop ALL existing policies first
DO $$ 
BEGIN
    -- Drop hotel_settings policies
    ...
    -- Drop account_categories policies
    ...
    -- Drop financial_reports policies
...
-- Inventory Policies
CREATE POLICY "inventory_staff_access_select" ON public.inventory
FOR SELECT USING (has_hotel_staff_access());

CREATE POLICY "inventory_management_insert" ON public.inventory
FOR INSERT WITH CHECK (has_management_access() OR (SELECT role FROM public.profiles WHERE id = auth.uid()) = 'procurement');

CREATE POLICY "inventory_management_update" ON public.inventory
FOR UPDATE USING (has_management_access() OR (SELECT role FROM public.profiles WHERE id = auth.uid()) = 'procurement')
