...
-- First, let's create a function to check if user has financial access rights
...
BEGIN
  ...
  IF public.has_financial_access() THEN
    INSERT INTO public.employee_access_log (
      ...
      NULL, -- No specific employee for financial data
      auth.uid(),
      'financial_' || lower(operation),
      ARRAY[table_name],
      now()
    );
  END IF;
...
CREATE TRIGGER financial_access_audit_account_entries
  AFTER SELECT ON public.account_entries
  FOR EACH ROW EXECUTE FUNCTION public.log_financial_access();

CREATE TRIGGER financial_access_audit_budgets
  AFTER SELECT ON public.budgets
  FOR EACH ROW EXECUTE FUNCTION public.log_financial_access();

CREATE TRIGGER financial_access_audit_financial_reports
  AFTER SELECT ON public.financial_reports
  FOR EACH ROW EXECUTE FUNCTION public.log_financial_access();